...
def evaluate(actor, env, memory=None, n_episodes=1, random1=False, noise=None, render=False):
    """
...
def train(n_episodes, output=None, debug=False, render=False):
    ...
    tnum = 'remote_evaluation/Average Returns'
    tnum1 = 'exploration/Average Returns'
    tnum2 = 'Epoch'
    n1 = s1 = s2 = 0
    mmd_states = []
    evaluationss  = []
    ...
    while total_steps < args.max_steps:
        ...
        random1 = total_steps < args.start_steps
        actor_steps = 0

        # training all agents
        for i in range(args.n_actor):
            f, s = evaluate(agent.actors[i], envs[i], n_episodes=n_episodes,
                            noise=a_noise, random1=random1, memory=memory, render=render)
            actor_steps += s
            total_steps += s
            step_cpt += s
        ...
        if step_cpt >= args.period:
            ...
            for i in range(args.n_actor):
                ...
                #fs.append(f)
                for i in range(s3):
                    returns_list = random.sample(f_list, s3)
                    episode_num2 = s2 + i
                    evaluationss.append([returns_list[i], returns_list[i], episode_num2])
                    test = pd.DataFrame(columns=[tnum, tnum1, tnum2], data=evaluationss)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--env', default='Ant-v2', type=str)
    ...
    # Training parameters
    parser.add_argument('--n_actor', default=10, type=int)
